...
namespace HireMate.Common.Utils
{
    public static class EmailHelper
    {
        ...
        {
            if (string.IsNullOrEmpty(timeZoneId)) { return string.Empty; }

            TimeZoneInfo timeZone = TimeZoneInfo.FindSystemTimeZoneById(timeZoneId);
            TimeSpan offset = timeZone.BaseUtcOffset;

            string offsetHours = offset.Hours.ToString("00");
            string offsetMinutes = offset.Minutes.ToString("00");

            string sign = offset.Hours >= 0 ? "+" : "-";

            return $"(GMT{sign}{offsetHours}:{offsetMinutes}) {timeZone.StandardName}";
        }
